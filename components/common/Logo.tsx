    ...
    <div className="flex items-center gap-3 group">
      ...
      {showText && (
        <div className="flex flex-col leading-tight">
          <h1
            className={`${textSizes[size]} font-black tracking-tighter ${light ? 'text-white' : 'text-slate-800'}`}
          >
            Regulariza<span className="text-blue-600">AI</span>
          </h1>
          {size !== 'sm' && (
            <span
              className={`text-[10px] font-bold uppercase tracking-[0.3em] ${light ? 'text-blue-200' : 'text-slate-400'}`}
            >
