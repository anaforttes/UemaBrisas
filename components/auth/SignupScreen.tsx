...
import { User, Mail, Lock, Eye, EyeOff, ShieldCheck, AlertCircle, Check, ArrowRight, Clock } from 'lucide-react';
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="w-full max-w-md bg-white border border-slate-200 rounded-[32px] shadow-2xl p-10 text-center">
          ...
          <p className="text-slate-500 font-medium text-sm leading-relaxed">
            ...
            O administrador irá aprovar o acesso e definir seu cargo em breve.
          </p>
          <Link to="/login" className="mt-8 inline-block text-blue-600 font-bold hover:underline text-sm">
            Voltar para o Login
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-[32px] shadow-2xl z-10 p-10">
        <h2 className="text-2xl font-black text-slate-800 mb-1 tracking-tight text-center">Solicitar Acesso</h2>
        <p className="text-slate-400 text-sm mb-6 font-medium text-center">Credenciamento de operadores do fluxo REURB-Doc</p>
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6">
          ...
          <p className="text-xs text-amber-700 leading-relaxed">
            Após o cadastro, sua conta ficará <strong>pendente de aprovação</strong>. O administrador definirá seu cargo e nível de acesso.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Crie uma senha</label>
              <div className="relative">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Confirme a senha</label>
              <div className="relative">
                <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input
          ...
          {form.password.length > 0 && (
            <div className="grid grid-cols-2 gap-2 px-1">
              {[
                ...
                <div key={label} className={`flex items-center gap-1.5 text-[10px] font-bold ${ok ? 'text-green-600' : 'text-slate-400'}`}>
                  {ok ? <Check size={11} /> : <div className="w-2.5 h-2.5 border border-slate-300 rounded-full" />}
                  {label}
                </div>
              ))}
            </div>
          ...
          >
            {loading ? 'Enviando...' : <><span>Solicitar Cadastro</span><ArrowRight size={18} /></>}
          </button>
        </form>
