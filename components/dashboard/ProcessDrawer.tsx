              ...
              {etapas.length > 0 ? (
                <div className="space-y-3 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                  {etapas.map((etapa, i) => (
                    <div key={i} className="flex gap-4 relative">
                      <div
                        className={`w-6 h-6 rounded-full border-4 border-white shadow-sm flex items-center justify-center z-10 shrink-0 ${ETAPA_COR[etapa.status] || 'bg-slate-200'}`}
                      >
                      ...
                      <div className="flex-1 pb-2">
                        <p
                          className={`text-sm font-bold ${etapa.status === 'pendente' ? 'text-slate-400' : 'text-slate-800'}`}
                        >
                ...
                <div className="space-y-6 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                  {steps.map((step, i) => (
                    <div key={i} className="flex gap-4 relative">
                      <div
                        className={`w-6 h-6 rounded-full border-4 border-white shadow-sm flex items-center justify-center z-10 ${
                          step.status === 'done'
                            ? 'bg-green-500'
                            : step.status === 'doing'
                              ? 'bg-blue-600'
                      ...
                      <div>
                        <p
                          className={`text-sm font-bold ${step.status === 'todo' ? 'text-slate-400' : 'text-slate-800'}`}
                        >
                          {step.label}
                        </p>
                        {step.status === 'doing' && (
              ...
              {documents.length === 0 ? (
                <div className="p-6 border-2 border-dashed border-slate-100 rounded-3xl text-center">
                  ...
                  <button
                    onClick={handleNovoDocumento}
                    disabled={criandoDoc}
                    className="mt-3 text-xs font-bold text-blue-600 hover:underline disabled:opacity-50"
                  >
                    {criandoDoc ? 'Criando...' : 'Criar primeiro documento'}
                  </button>
                </div>
              ) : (
          ...
          {abaAtiva === 'anexos' && (
            <section>
              ...
              >
                <Upload
                  size={24}
                  className={`mx-auto mb-2 ${dragAtivo ? 'text-blue-500' : 'text-slate-300'}`}
                />
                <p className="text-xs font-bold text-slate-500">
                  Arraste arquivos ou clique para selecionar
                </p>
                <p className="text-[10px] text-slate-400 mt-1">
                  PDF, DOC, DOCX, JPG, PNG — até 5MB por arquivo
                </p>
              </div>
              ...
              {anexos.length === 0 ? (
                <div className="p-6 border-2 border-dashed border-slate-100 rounded-3xl text-center">
                  ...
                  <p className="text-[10px] text-slate-300 mt-1">
                    Plantas, memoriais, laudos, fotos
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  {anexos.map((anexo) => (
                    <div
                      key={anexo.id}
                      className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-white hover:border-slate-200 transition-all group"
                    >
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  ...
                  <p className="text-sm font-black text-slate-800">
                    {formatarData(process.createdAt)}
                  </p>
                </div>
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-black text-slate-800">
                    {formatarData(process.updatedAt)}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-blue-50/30 border border-blue-50 rounded-2xl">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-800">
                    {process.responsibleName || 'Não atribuído'}
                  </p>
              ...
              {!process.responsibleName && (
                <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-100 rounded-xl">
                  ...
                  <p className="text-xs text-amber-700">
                    Nenhum responsável atribuído a este processo.
                  </p>
                </div>
              )}
            </section>
