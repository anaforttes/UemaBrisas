...
export const ProcessManagement: React.FC = () => {
  ...
  const stats = [
    ...
    {
      label: 'Diligências',
      value: processes.filter((p) => p.status === ProcessStatus.DILIGENCIA).length,
      color: 'text-red-500',
      icon: AlertCircle,
      bg: 'bg-red-50',
    },
    {
      ...
      value: processes.filter(
        (p) => p.status === ProcessStatus.CONCLUIDO || p.status === ProcessStatus.FINALIZADO
      ).length,
    ...
    <div className="p-6 lg:p-10 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium mt-1">
            Gerencie o fluxo de regularização da prefeitura.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="bg-white border border-slate-100 p-6 rounded-[28px] shadow-sm flex items-center gap-5 group hover:shadow-md transition-all"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredProcesses.map((proc) => {
            ...
            return (
              <div
                key={proc.id}
                className="bg-white border border-slate-200 rounded-[32px] p-6 hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 group"
              >
                    ...
                    <div className="flex flex-wrap gap-1 justify-end">
                      {meuPapel.meus_papeis &&
                        ...
                        meuPapel.meus_papeis.map((papel) => (
                          <span
                            key={papel}
                            className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full border ${PAPEL_STYLE[papel] ?? 'bg-slate-50 text-slate-500 border-slate-100'}`}
                          >
                            {papel}
                          </span>
                        ))}
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    <span className="flex items-center gap-1.5">
                      <Activity size={10} /> Progresso Geral
                    </span>
                  ...
                  {pode.verMenuAcoes && (
                    <div className="relative" ref={menuAberto === proc.id ? menuRef : null}>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setMenuAberto(menuAberto === proc.id ? null : proc.id);
                        }}
          ...
          {filteredProcesses.length === 0 && (
            <div className="col-span-full py-32 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-100">
              ...
              <p className="text-slate-400 text-sm font-medium mt-1">
                Ajuste os filtros ou crie um novo protocolo.
              </p>
            </div>
          )}
        </div>
      ...
      <ProcessDrawer process={processoSelecionado} onClose={() => setProcessoSelecionado(null)} />
    </div>
  );
};
