            ...
            <tr key={proc.id} className="hover:bg-slate-50/80 transition-all group cursor-pointer">
              <td className="px-8 py-6">
                ...
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">
                  {proc.id} • {proc.applicant}
                </div>
              </td>
              <td className="px-8 py-6">
                <span
                  className={`text-[10px] px-3 py-1.5 rounded-full font-black tracking-widest ${proc.modality === 'REURB-S' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}
                >
                  {proc.modality}
                </span>
              </td>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2.5 h-2.5 rounded-full animate-pulse ${
                      proc.status === ProcessStatus.FINALIZADO
                        ? 'bg-green-500'
                        : proc.status === ProcessStatus.ANALISE_JURIDICA
                          ? 'bg-amber-500'
                          : 'bg-blue-500'
                    }`}
                  />
              ...
              <td className="px-8 py-6 text-right">
                <Link
                  to={`/edit/${proc.id}`}
                  className="p-3 bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:shadow-lg transition-all inline-flex"
                >
