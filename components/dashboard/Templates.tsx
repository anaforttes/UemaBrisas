...
const PLACEHOLDER_HINTS: Record<string, string> = {
  ...
  local: 'Ex: Rua das Flores, São Luís - MA',
  area: 'Ex: 12.500 m²',
  municipio: 'Ex: São Luís',
  estado: 'Ex: MA',
...
function normalizarFraseProfissional(texto: string): string {
  return texto
    .replace(/,\s*no cargo de\s*Nao possui profissao\s*,/gi, ', sem profissão declarada,')
    .replace(/,\s*no cargo de\s*Autonomo\(a\)\s*,/gi, ', autônomo(a),')
    .replace(/,\s*no cargo de\s*Aposentado\(a\)\s*,/gi, ', aposentado(a),')
    .replace(/,\s*Nao possui profissao\s*,/gi, ', sem profissão declarada,')
    .replace(/,\s*Autonomo\(a\)\s*,/gi, ', autônomo(a),')
...
const ModalDadosDocumento: React.FC<ModalDadosDocumentoProps> = ({
  ...
  const groupedCampos = useMemo(() => {
    ...
    return {
      ...
      'Dados do documento': camposVisiveis.filter((c) => c.categoria === 'Dados do documento'),
    };
  }, [campos]);
  ...
  const handleSituacaoProfissional = (situacao: SituacaoProfissional) => {
    ...
    if (situacao === 'padrao') {
      setDados((prev) => {
        const cargosAuto = ['Autônomo(a)', 'Não possui profissão', 'Aposentado(a)'];
        return { ...prev, cargo: cargosAuto.includes(prev.cargo || '') ? '' : prev.cargo };
      });
      return;
    }
    const cargoPorSituacao: Record<Exclude<SituacaoProfissional, 'padrao'>, string> = {
      autonomo: 'Autônomo(a)',
      sem_profissao: 'Não possui profissão',
      aposentado: 'Aposentado(a)',
    };
    setDados((prev) => ({ ...prev, cargo: cargoPorSituacao[situacao] }));
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col">
        <div className="border-b border-slate-100 flex items-center justify-between p-6">
          <div>
            <h3 className="text-lg font-black text-slate-800">Geração Dinâmica de Documento</h3>
            <p className="text-sm text-slate-400 font-medium mt-1">
