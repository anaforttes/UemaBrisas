...
export const exportarDOCX = async (titulo: string, conteudoHtml: string): Promise<void> => {
  ...
  const parseNo = (no: ChildNode): TextRun[] => {
    if (no.nodeType === 3) {
    ...
    const opcoes = (r: TextRun) => (r as unknown as { options: Record<string, unknown> }).options;
    switch (el.tagName?.toLowerCase()) {
      case 'b':
      case 'strong':
        return filhos().map((r) => new TextRun({ ...opcoes(r), bold: true }));
      case 'i':
      case 'em':
        return filhos().map((r) => new TextRun({ ...opcoes(r), italics: true }));
      case 'br':
  ...
  const parseBloco = (el: Element): (Paragraph | DocxTable)[] => {
  ...
  const conteudo: (Paragraph | DocxTable)[] = [
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [
        new TextRun({
