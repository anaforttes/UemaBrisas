...
export const SignupScreen = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await dbService.users.insert(form);
      setSuccess(true);
          ...
          <>
            ...
            <form onSubmit={handleSignup} className="space-y-8">
              ...
              <div className="pt-4 flex flex-col gap-6">
                <button
                  type="submit"
                  disabled={((!isPasswordValid || !passwordsMatch) && (form.password.length > 0)) || loading}
                  className={`w-full py-5 rounded-[24px] font-black text-sm uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 shadow-2xl ${isPasswordValid && passwordsMatch
                      ? 'bg-blue-600 text-white shadow-blue-200 hover:bg-blue-700 hover:-translate-y-1'
                      : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'
                    }`}
                >
                  {loading ? 'Processando...' : 'Solicitar Cadastro'} {!loading && <ArrowRight size={20} />}
                </button>
