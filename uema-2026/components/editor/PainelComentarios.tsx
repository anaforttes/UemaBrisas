...
const PainelComentarios: React.FC<PainelComentariosProps> = ({ nomeUsuario, cargoUsuario, editor }) => {
  ...
  if (decisao === 'aceito' && comentario?.tipo === 'sugestao' && comentario.textoSugerido && editor) {
    const { state } = editor;
    const { doc } = state;
    let found = false;
    ...
    doc.descendants((node: any, pos: number) => {
      ...
      const textoOriginal = comentario.texto
        .replace('Sugiro alterar "', '')
        .split('" por "')[0];

      if (node.text?.includes(textoOriginal)) {
        const inicio = pos + node.text.indexOf(textoOriginal);
        const fim = inicio + textoOriginal.length;
        editor.chain()
          .focus()
          .setTextSelection({ from: inicio, to: fim })
          .insertContent(comentario.textoSugerido!)
          .run();
        found = true;
      }
    });
