...
type AbaAtiva = 'comentarios' | 'historico';
  ...
  return (
    <>
      ...
      <SignatureModal
        ...
        onSignatureComplete={(record) => { setRegistroAssinatura(record); onSave(editor.getHTML(), tituloLocal, 'Signed'); registrarEvento('assinatura', `Assinado — Protocolo: ${record.protocol}`); }}
      />
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-slate-100 bg-white sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2 mr-1">
              {membros.slice(0, 2).map((u, i) => (
                <div key={i} title={u.name} className={`w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-bold text-white ${i === 0 ? 'bg-blue-500' : 'bg-green-500'}`}>{u.name.charAt(0)}</div>
              ))}
            </div>
